...
public class State2<VQ extends Vertex, VG extends Vertex, EQ extends Edge, EG extends Edge> {
	 ...
	 * graphs.
	 */
	public State2(Graph<VQ, EQ> p, int v1, Graph<VG, EG> g, int v2,
			Map<String, Quantifier> quantifiers, QuantifierCheckMatrix m, boolean checkQuantifiers,
			boolean isPositive) {
		this.p = p;
		this.g = g;

		// FIXME: should be positive?
		this.checkQuantifiers = checkQuantifiers && isPositive;
	...
	@SuppressWarnings("unchecked")
	public State2(State2 copy, Graph<VQ, EQ> ngGraph) {
		quantifiers = copy.quantifiers;
		checkQuantifiers = copy.checkQuantifiers;
		p = copy.p.copy();
		g = copy.g;
